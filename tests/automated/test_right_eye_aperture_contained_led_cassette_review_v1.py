...
HERE = ROOT / "hardware/mechanical/fabrication/3d-print/cat-head-full-size-v1/source/cad-change-control/review-only/right-eye-aperture-contained-led-cassette-review-v1"
CONTRACT = json.loads((HERE / "contract.json").read_text(encoding="utf-8"))
SOURCE = (HERE / "generate_review.py").read_text(encoding="utf-8")


class ApertureContainedLedCassetteReviewTests(unittest.TestCase):
    ...
    def test_front_is_inset_and_has_no_surface_flange(self) -> None:
        self.assertGreaterEqual(CONTRACT["dimensions_mm"]["front_perimeter_radial_inset"], 2.0)
        self.assertGreaterEqual(CONTRACT["dimensions_mm"]["rear_perimeter_radial_inset"], 6.0)
        self.assertGreaterEqual(CONTRACT["dimensions_mm"]["visible_aperture_radial_inset"], 4.2)
        self.assertIn("no exterior flange", CONTRACT["architecture"]["head_attachment"])
