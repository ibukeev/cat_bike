...
**Next active work:** `HS-11`. V11 was visually approved on 2026-08-14. It preserves the user-accepted V10 actual
outer-neck deletion and restores the exact visually approved V2 C046/C048
clearance geometry that V10 accidentally regressed. C046/C048 clear the V9 eye
by `4.6063/4.0317 mm`; both flange-pair gaps remain `0.3000 mm` with zero
interference. V12 isolated the inherited revised lower-face mesh into `60` unchanged
components and mapped the primary owner's `41` intersections to `21` legacy
seam/slot face pairs. FreeCAD fusion and both Blender self-union solvers were
...
`HS-04` now has an isolated orientation proposal ready for visual review. The
proposed C001 placement measures `194.385 x 172.420 x 164.737 mm`, retains
`22.808/13.790 mm` XY reserve per side, and creates no production export. The
orientation must be explicitly approved before the exact ASA coupon is built.
...
| HS-04 | [ ] | Produce exact-orientation ASA short-insert coupon | The isolated V1 orientation proposal passes the conservative envelope and 10 mm reserve audit at `194.385 x 172.420 x 164.737 mm`; A/B axes are `62.385/39.288 deg` from the layer normal. No production export exists. | User visually approves the V1 bed placement; then export/slice the A/B cavity coupon in that exact orientation. |
...
| HS-11 | [ ] | Integrate and validate all eight eye flanges | V11 was visually approved on 2026-08-14 and freezes the combined accepted right-side state without moving the eye or four flanges: C046/C048 clearance is `4.6063/4.0317 mm`, both pair gaps are `0.3000 mm`, and the stale original ribs are absent. V12 maps `41` intersections to `21` inherited seam/slot face pairs; automatic fusion/self-union variants are rejected. | Repartition only the V12-mapped inherited faces without changing the exterior, integrate copied owners, revalidate hardware/clearance/exterior, mirror exactly, and repeat bilaterally. |
...
- [Right A/B print-orientation review V1 checkpoint](RIGHT_AB_PRINT_ORIENTATION_REVIEW_V1_CHECKPOINT_2026-08-14.md)
