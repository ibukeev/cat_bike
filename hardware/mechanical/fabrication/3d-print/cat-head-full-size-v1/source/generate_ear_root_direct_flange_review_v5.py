#!/usr/bin/env python3
"""Generate the F-10/F-11/F-12 direct under-ear flange review."""
...
DEFAULT_CONFIG = PACKAGE_ROOT / "config/ear-root-direct-flange-review-v5.json"
...
def configure_scene(output_dir: Path, resolution_px: int) -> bpy.types.Object:
    ...
    scene.name = "Ear_Root_Direct_Flange_Review_V5"
    ...
    camera_data = bpy.data.cameras.new("EAR5_REVIEW_ONLY__Camera")
    camera = bpy.data.objects.new("EAR5_REVIEW_ONLY__Camera", camera_data)
...
def render_view(
    ...
    path = output_dir / "renders" / f"ear-root-direct-flange-{name}.png"
...
def transverse_convex_hull(
    ...
    if len(projected) < 3:
        raise ValueError(
            "Direct-flange cross-section has fewer than three unique points"
        )
...
def direct_flange_tab(
    ...
    half_tangent = float(values["direct_flange_tab_tangent_length_mm"]) / 2.0
    shell_radial_min = float(values["direct_flange_tab_shell_radial_min_mm"])
    shell_radial_max = float(values["direct_flange_tab_shell_contact_radial_max_mm"])
    insert_radial_min = float(values["direct_flange_tab_insert_root_radial_min_mm"])
    insert_radial_max = float(values["direct_flange_tab_insert_radial_max_mm"])
    insert_outer = float(values["direct_flange_tab_outer_depth_mm"])
    shell_outer = float(values["direct_flange_tab_shell_side_outer_depth_mm"])
    thickness = float(values["direct_flange_tab_thickness_mm"])
    lead = float(values["direct_flange_lead_in_mm"])
    candidates = [
        *[
...
def create_point_geometry(
    ...
    insert_flange_root = ranged_box(
        f"EAR5_DIRECT_FLANGE_ROOT__right__{point_id}",
        anchor,
        tangent,
        insert_radial,
        insert_inward,
    ...
    shell_anchor = ranged_box(
        f"EAR5_FIXED_SHELL_ANCHOR__right__{point_id}",
        anchor,
        tangent,
        shell_radial,
        shell_inward,
    ...
    cut_axis_cavity(
        shell_anchor,
        f"EAR5_HEAT_SET_CAVITY__right__{point_id}",
        heat_center,
        inward,
    ...
    direct_tab = direct_flange_tab(
        f"EAR5_DIRECT_FLANGE_TAB__right__{point_id}",
        anchor,
        tangent,
        insert_radial,
        insert_inward,
    ...
    root_tab_intersections = ear_v3.world_triangle_intersection_count(
        insert_flange_root, direct_tab
    )
    if root_tab_intersections == 0:
        raise ValueError(
            f"right {point_id} direct-flange tab has no root overlap"
        )
    gate5.apply_boolean(
        insert_flange_root, direct_tab, "UNION", solver="EXACT"
    )
    insert_flange = insert_flange_root
    insert_flange.name = f"EAR5_DIRECT_FLANGE__right__{point_id}"
    gate5.require_manifold(
        insert_flange, f"right {point_id} unified direct flange"
    )

    tab_shell_outer = float(
        values["direct_flange_tab_shell_side_outer_depth_mm"]
    )
    tab_thickness = float(values["direct_flange_tab_thickness_mm"])
    tab_hole_center = (
        anchor
        + radial * screw_radial
        + inward * (tab_shell_outer + tab_thickness / 2.0)
    )
    gate6.cut_axis_hole(
        insert_flange,
        f"EAR5_M3_CLEARANCE__right__{point_id}",
        tab_hole_center,
        inward,
    ...
    gate5.require_manifold(
        insert_flange, f"right {point_id} drilled direct flange"
    )

    heat_insert = gate5.cylinder(
        f"EAR5_HARDWARE__right__{point_id}__heat_set_insert",
        anchor
        + radial * screw_radial
    ...
    shaft = gate5.cylinder(
        f"EAR5_HARDWARE__right__{point_id}__m3_shaft",
        anchor + radial * screw_radial + inward * (anchor_inner - 3.0),
        anchor
        + radial * screw_radial
        + inward * (tab_shell_outer + tab_thickness + 0.8),
        3.0,
        materials["hardware"],
        vertices=24,
    )
    washer_start = tab_shell_outer + tab_thickness
    washer = gate5.cylinder(
        f"EAR5_HARDWARE__right__{point_id}__washer",
    ...
    head = gate5.cylinder(
        f"EAR5_HARDWARE__right__{point_id}__button_head",
    ...
    tool = gate5.cylinder(
        f"EAR5_ACCESS__right__{point_id}__tool_corridor",
    ...
    finger = gate5.cylinder(
        f"EAR5_ACCESS__right__{point_id}__finger_envelope",
        anchor + radial * screw_radial + inward * access_start,
        anchor
        + radial * screw_radial
    ...
    return {
        ...
        "direct_flange_root_tab_intersection_pairs": root_tab_intersections,
...
def union_composite(
    ...
    for index, flange in enumerate(flanges, start=1):
        ...
        gate5.apply_boolean(composite, tool, "UNION", solver="MANIFOLD")
        incremental_topology = topology_record(composite)
        if (
            incremental_topology["connected_components"] != 1
            or incremental_topology["boundary_edges"]
            or incremental_topology["nonmanifold_edges"]
        ):
            raise ValueError(
                f"{name} failed after flange {flange.name}: "
                f"{incremental_topology}"
            )
...
def validate_path(
    ...
    retention_margin = float(
        v5_config["retention"]["moving_retention_path_margin_mm"]
    )
    deep_envelope = gate8.expanded_insert_cutter(
        deep_body, body_margin, f"EAR5_{side}_deep_body_margin"
    )
    retention_envelopes = [
        gate8.expanded_insert_cutter(
            retainer, retention_margin, f"EAR5_{side}_retention_margin_{index}"
        )
    ...
    for sample in samples:
        ...
        seated = abs(float(sample["translation_mm"])) < 0.000001
        active_targets = structural_path_targets
        if not seated:
            active_targets = [
                *structural_path_targets, *fixed_path_targets
            ]
        actual_hits = collision_hits(composite, active_targets)
        deep_hits = collision_hits(deep_envelope, active_targets)
        margin_hits = {}
        for envelope in retention_envelopes:
            for target_name, count in collision_hits(
    ...
    if deep_conflicts:
        raise ValueError(
            f"{side} accepted V3 deep body path regressed: {deep_conflicts}"
        )
    direct_flange_path_clear = not (
        conflicts or moving_margin_conflicts
    )
    return (
        {
            "side": side,
            "screws_and_washers_removed_during_path": True,
            "seated_expected_bearing_contact_exempt_from_fixed_target_margin": True,
            ...
            "direct_flange_path_conflicts": conflicts,
            "moving_retention_margin_conflicts": moving_margin_conflicts,
            "accepted_v3_deep_body_path_clear": True,
            "direct_flange_path_clear": direct_flange_path_clear,
            "upper_head_release_required_for_insert_service": not direct_flange_path_clear,
        },
        samples,
    )
...
def main() -> None:
    ...
    right_deep = ear_v2.deserialize_mesh(
        "EAR5_VALIDATION__right_deep_fit_body", source_payloads["deep"]
    )
    bpy.context.scene.collection.objects.link(right_deep)
    right_body = ear_v2.deserialize_mesh(
        "EAR5_ACCEPTED_V3_BODY__right", source_payloads["full"]
    )
    ...
    left_deep = mirror_object(
        right_deep, "EAR5_VALIDATION__left_deep_fit_body"
    )
    left_body = mirror_object(
        right_body, "EAR5_ACCEPTED_V3_BODY__left"
    )
    ...
    for right_point in point_records["right"]:
        ...
        left_point = {
            ...
            "insert_flange": mirror_object(
                ...
                f"EAR5_DIRECT_FLANGE__left__{point_id}",
                materials["insert_flange"],
            ),
            "direct_flange_root_tab_intersection_pairs": right_point["direct_flange_root_tab_intersection_pairs"],
            "fixed_shell": mirror_object(
                right_point["fixed_shell"],
                f"EAR5_FIXED_SHELL_ANCHOR__left__{point_id}",
                materials["fixed_shell"],
            ),
            "hardware": [
                mirror_object(
    ...
    for side in ("left", "right"):
        ...
        composite, overlaps = union_composite(
            bodies[side], flanges, f"EAR5_VALIDATION__{side}_moving_composite"
        )
    ...
    for side in ("left", "right"):
        ...
        seated_hits = collision_hits(
            composites[side], structural_targets
        )
        if seated_hits:
        ...
        direct_flange_unintended_fixed_hits = {}
        retention = config["retention"]
        fixed_contact_gap = float(
            retention["direct_flange_tab_shell_side_outer_depth_mm"]
        ) - (
            float(retention["shell_anchor_outer_depth_mm"])
            + float(retention["shell_anchor_thickness_mm"])
        )
        fixed_tangent_overlap = min(
            float(retention["direct_flange_tab_tangent_length_mm"]),
            float(retention["shell_anchor_tangent_length_mm"]),
        )
        fixed_radial_overlap = min(
            float(retention["direct_flange_tab_shell_contact_radial_max_mm"]),
            float(retention["shell_anchor_radial_max_mm"]),
        ) - max(
            float(retention["direct_flange_tab_shell_radial_min_mm"]),
            float(retention["shell_anchor_radial_min_mm"]),
        )
        if abs(fixed_contact_gap) > 0.001 or fixed_radial_overlap <= 0.0:
            raise ValueError("Direct flange does not seat on its shell boss")
        fixed_contact_area = fixed_tangent_overlap * fixed_radial_overlap
        expected_fixed_contact_areas = {}
        for point in point_records[side]:
            direct_flange = point["insert_flange"]
            expected_fixed = point["fixed_shell"]
            expected_fixed_contact_areas[direct_flange.name] = round(
                fixed_contact_area, 4
            )
            unintended_fixed_targets = [
                fixed for fixed in all_fixed if fixed != expected_fixed
            ]
            hits = collision_hits(direct_flange, unintended_fixed_targets)
            if hits:
                direct_flange_unintended_fixed_hits[direct_flange.name] = hits

        if direct_flange_unintended_fixed_hits:
            raise ValueError(
                f"{side} direct flanges hit other fixed bosses: "
                f"{direct_flange_unintended_fixed_hits}"
            )
        access_hits = {}
        access_targets = [
            target
            for target in [*structural_targets, *all_fixed]
        ...
        if minimum_separation < float(
            ...
            raise ValueError(f"{side} direct flanges are not spatially separated")
        side_reports[side] = {
            ...
            "direct_flange_root_tab_intersection_pairs": [
                point["direct_flange_root_tab_intersection_pairs"]
                for point in point_records[side]
            ],
            "direct_flange_unintended_fixed_anchor_hits": direct_flange_unintended_fixed_hits,
            "direct_flange_shell_bearing_area_mm2": expected_fixed_contact_areas,
            "direct_flange_shell_bearing_gap_mm": round(fixed_contact_gap, 4),
            "direct_flange_shell_radial_overlap_mm": round(fixed_radial_overlap, 4),
            "access_envelope_hits_with_ear_removed": access_hits,
            "direct_flange_axes_dot_local_inward": [
            ...
            "points": [
                {
                    ...
                    "moving_flange_has_standard_m3_clearance_hole": True,
                    "direct_screw_removed_before_insert_service": True,
                }
                for point in point_records[side]
            ],
    ...
    collection_names = (
        "EAR5_EXACT_EARS_CYAN__UNCHANGED",
        "EAR5_EXACT_UPPER_HEADS_GRAY__UNCHANGED",
        "EAR5_ACCEPTED_V3_BODIES_YELLOW__UNCHANGED",
        "EAR5_DIRECT_FLANGES_ORANGE__PROPOSED",
        "EAR5_FIXED_SHELL_ANCHORS_GREEN__PROPOSED_NOT_INTEGRATED",
        "EAR5_EXACT_LOWER_AND_REAR_SHELLS_MUTED__UNCHANGED",
        "EAR5_M3_HARDWARE_BRASS__PROPOSED",
        "EAR5_ACCESS_ENVELOPES_WHITE__HIDDEN_BY_DEFAULT",
        "EAR5_OTHER_SOURCE_GEOMETRY__HIDDEN",
    )
    collections = {}
    ...
    for side in ("left", "right"):
        ...
        link_reference(ear, collections["EAR5_EXACT_EARS_CYAN__UNCHANGED"])
        link_reference(
            upper, collections["EAR5_EXACT_UPPER_HEADS_GRAY__UNCHANGED"]
        )
        link_reference(
            bodies[side],
            collections["EAR5_ACCEPTED_V3_BODIES_YELLOW__UNCHANGED"],
        )
        for obj in [
            ear,
            upper,
        ...
        for point in point_records[side]:
            link_reference(
                point["insert_flange"],
                collections["EAR5_DIRECT_FLANGES_ORANGE__PROPOSED"],
            )
            link_reference(
                point["fixed_shell"],
                collections[
            ...
            for obj in point["hardware"]:
                link_reference(
                    obj, collections["EAR5_M3_HARDWARE_BRASS__PROPOSED"]
                )
    ...
    for obj in source_set:
        if obj in all_visible:
            continue
        link_reference(
            obj, collections["EAR5_OTHER_SOURCE_GEOMETRY__HIDDEN"]
        )
        obj.hide_viewport = True
        obj.hide_render = True
        obj.hide_set(True)
    ...
    for side, sign in (("left", -1.0), ("right", 1.0)):
        ...
        renders.append(
            render_view(
                camera,
                output_dir,
                f"{side}-retention-context",
                review_location,
                target,
                side_visible[side] - {bodies[side]},
            )
        )
        renders.extend(
            [
                ...
                render_view(
                    camera,
                    output_dir,
                    f"{side}-insertion-ready-screws-removed",
                    review_location,
                    target,
                    insertion_ready_visible[side],
                ),
    ...
    if protected_before != protected_after:
        raise ValueError("V5 review changed exact Gate 8 source geometry")
    scene = bpy.context.scene
    ...
    scene["direct_screws_removed_before_upper_head_release"] = True
    scene["blue_removable_clamp_count"] = 0
    ...
    blend_path = output_dir / "ear-root-direct-flange-review-v5.blend"
    ...
    report = {
        ...
        "moving_insert_flanges_have_standard_m3_clearance_holes": True,
        "direct_screws_and_washers_removed_before_upper_head_release": True,
        "manual_hole_enlargement_allowed_with_washer_and_edge_check": True,
        "blue_removable_clamp_count": 0,
        "direct_m3_fastener_count": 6,
    ...
    report_path = output_dir / "ear-root-direct-flange-review-v5-validation.json"
