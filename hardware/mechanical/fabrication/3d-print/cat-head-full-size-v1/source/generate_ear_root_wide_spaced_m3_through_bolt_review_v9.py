#!/usr/bin/env python3
"""Generate two wide-spaced, M3-drilled connector sets per translucent piece."""
...
DEFAULT_CONFIG = (
    PACKAGE_ROOT / "config/ear-root-wide-spaced-m3-through-bolt-review-v9.json"
)
...
def configure_scene(output_dir: Path, resolution_px: int) -> bpy.types.Object:
    ...
    scene.name = "Ear_Root_Internal_Rectangular_Flange_Placement_Review_V9"
    ...
    camera_data = bpy.data.cameras.new("EAR9_REVIEW_ONLY__Camera")
    camera = bpy.data.objects.new("EAR9_REVIEW_ONLY__Camera", camera_data)
...
def render_view(
    ...
    path = (
        output_dir
        / "renders"
        / f"ear-root-wide-spaced-m3-through-bolt-{name}.png"
    )
...
def create_pair(
    ...
    orange = gate5.box(
        f"EAR9_{label}_INSERT_FLANGE__orange",
        orange_center,
        axes,
        dimensions,
        materials["orange"],
    ...
    green = gate5.box(
        f"EAR9_{label}_HEAD_FLANGE__green",
        green_center,
        axes,
        dimensions,
        materials["green"],
    ...
    if actual_minimum_hole_edge < minimum_hole_edge:
        raise ValueError(
            f"V9 {label} M3 bore edge material is "
            f"{actual_minimum_hole_edge:.3f} mm, below {minimum_hole_edge:.3f} mm"
        )
    for role, tab in (("orange", orange), ("green", green)):
        gate6.cut_axis_hole(
            tab,
            f"EAR9_{label}_{role.upper()}__m3_clearance",
            hole_center,
            across,
            hole_diameter,
            hole_span,
        )
        gate5.require_manifold(tab, f"V9 {label} drilled {role} tab")
    ...
    if c002_v2.surfaces_overlap(orange, green):
        raise ValueError(f"V9 {label} rectangular tabs overlap")
    measured_gap = c002_v2.surface_distance(orange, green)
    if abs(measured_gap - gap) > 0.01:
        raise ValueError(
            f"V9 {label} placement gap changed: expected {gap}, got {measured_gap}"
        )
    ...
    if orange_owner_hits == 0:
        raise ValueError(f"V9 {label} orange tab does not root into moving owner")
    if green_owner_hits == 0:
        raise ValueError(f"V9 {label} green tab does not root into fixed owner")
...
def mirror_pair_from_right(
    ...
    for role, material_name in (("orange", "orange"), ("green", "green")):
        ...
        mirrored = ear_v2.deserialize_mesh(
            f"EAR9_LEFT_{location_id}_{role.upper()}_FLANGE__{role}",
    ...
    if orange_hits == 0 or green_hits == 0:
        raise ValueError(
            f"V9 LEFT_{location_id} mirrored owner root missing: "
...
def validate_open_aligned_bore(pair: dict[str, Any]) -> dict[str, Any]:
    ...
    gauge = gate5.cylinder(
        f"EAR9_VALIDATION__{pair['side']}_{pair['location_id']}_m3_bore_gauge",
    ...
    if any(hits.values()):
        raise ValueError(
            f"V9 {pair['side']} {pair['location_id']} M3 bore is obstructed: {hits}"
        )
    return {
        "side": pair["side"],
...
def create_boolean_proof_part(
    ...
    if not result.data.polygons or gate5.mesh_volume(result) <= 0.001:
        raise ValueError(f"V9 review proof is empty: {name}")
...
def fuse_owner_with_tabs(
    ...
    if (
        ...
        raise ValueError(f"V9 owner/tab composite is not manifold: {topology}")
...
def validate_side_path(
    ...
    deep_envelope = gate8.expanded_insert_cutter(
        deep_body, deep_margin, f"EAR9_VALIDATION__{side}_deep_body_margin"
    )
    flange_envelopes = []
    for orange in orange_tabs:
        envelope = gate8.expanded_insert_cutter(
            orange,
            flange_margin,
            f"EAR9_VALIDATION__{orange.name}_margin",
        )
    ...
    if actual_conflicts:
        raise ValueError(f"V9 {side} actual path collides: {actual_conflicts}")
    return {
        "side": side,
...
def main() -> None:
    ...
    right_deep = ear_v2.deserialize_mesh(
        "EAR9_VALIDATION__right_deep_fit_body", source_payloads["deep"]
    )
    bpy.context.scene.collection.objects.link(right_deep)
    right_body = ear_v2.deserialize_mesh(
        "EAR9_ACCEPTED_V3_BODY__right", source_payloads["full"]
    )
    ...
    left_deep = ear_v2.deserialize_mesh(
        "EAR9_VALIDATION__left_deep_fit_body",
    ...
    if len(location_specs) != 2:
        raise ValueError("V9 requires exactly two connector locations per side")
    ...
    for spec in location_specs:
        ...
        if right_record["owner"] != "right_upper_head":
            raise ValueError(
                f"V9 right {spec['id']} owner changed: {right_record['owner']}"
            )
        ...
        if left_record["owner"] != "left_upper_head":
            raise ValueError(
                f"V9 left {spec['id']} owner changed: {left_record['owner']}"
            )
        ...
        if mirrored_anchor_error > 0.01:
            raise ValueError(
                f"V9 left {spec['id']} seam anchor is not mirrored: "
    ...
    if len(pairs) != 4:
        raise ValueError("V9 must contain four connector sets total")
    separations = {}
    ...
    for side, side_pairs in pairs_by_side.items():
        ...
        if separation < minimum_separation:
            raise ValueError(
                f"V9 {side} connector sets are clustered: {separation:.3f} mm"
            )
    mirror_errors = {}
    for spec in location_specs:
        right_pair = next(
        ...
        if error > 0.01:
            raise ValueError(
                f"V9 location {spec['id']} is not mirrored: {error:.4f} mm"
            )
    ...
    if len(bore_reports) != 4 or not all(
        ...
        raise ValueError("V9 requires four open, aligned M3 fastener paths")
    ...
    for pair in pairs:
        ...
        prefix = f"EAR9_REVIEW_ONLY__{key}"
        parts = {
        ...
        for owner_key in ("orange_owner_overlap", "green_owner_overlap"):
            ...
            if volume < minimum_overlap:
                raise ValueError(
                    f"V9 weak owner root {volume_key}: {volume:.3f} mm3"
                )
    ...
    for side in ("right", "left"):
        ...
        composite, topology = fuse_owner_with_tabs(
            bodies[side],
            orange_tabs,
            f"EAR9_VALIDATION__{side}_body_plus_two_orange_tabs",
        )
        ...
        if side_seated_hits:
            raise ValueError(
                f"V9 {side} moving composite hits shells: {side_seated_hits}; "
        ...
        if per_green_hits:
            raise ValueError(
                f"V9 {side} green tabs hit unintended shells: {per_green_hits}"
            )
    ...
    collection_names = (
        "EAR9_EXACT_STRUCTURAL_HEAD_MUTED__UNCHANGED",
        "EAR9_EXACT_EARS_CYAN__UNCHANGED",
        "EAR9_ACCEPTED_V3_TRANSLUCENT_BODIES_YELLOW__UNCHANGED",
        "EAR9_RIGHT_INSERT_FLANGES_ORANGE__TWO_SETS",
        "EAR9_LEFT_INSERT_FLANGES_ORANGE__TWO_SETS",
        "EAR9_RIGHT_HEAD_FLANGES_GREEN__TWO_SETS_UNINTEGRATED",
        "EAR9_LEFT_HEAD_FLANGES_GREEN__TWO_SETS_UNINTEGRATED",
        "EAR9_REVIEW_ONLY__OWNER_ROOT_BOOLEAN_CUTAWAY_PROOFS__HIDDEN",
        "EAR9_OTHER_SOURCE_GEOMETRY__HIDDEN",
    )
    ...
    for target in structural_targets:
        ...
        c002_v2.link_reference(
            target, collections["EAR9_EXACT_STRUCTURAL_HEAD_MUTED__UNCHANGED"]
        )
    ...
    for ear in ears.values():
        ...
        ear.hide_set(False)
        c002_v2.link_reference(
            ear, collections["EAR9_EXACT_EARS_CYAN__UNCHANGED"]
        )
    for body in bodies.values():
        c002_v2.link_reference(
            body,
    ...
    for pair in pairs:
        ...
        c002_v2.link_reference(
            pair["orange"],
            collections[f"EAR9_{side_label}_INSERT_FLANGES_ORANGE__TWO_SETS"],
        )
        c002_v2.link_reference(
            pair["green"],
            collections[
                f"EAR9_{side_label}_HEAD_FLANGES_GREEN__TWO_SETS_UNINTEGRATED"
            ],
        )
    ...
    for obj in source_set:
        if obj in default_visible:
            continue
        c002_v2.link_reference(
            obj, collections["EAR9_OTHER_SOURCE_GEOMETRY__HIDDEN"]
        )
    ...
    for view_name, (baseline_render, candidate_render) in (
        ...
        if not comparison["identical"]:
            raise ValueError(
                f"V9 changes the {view_name} exterior silhouette: {comparison}"
            )
    ...
    if protected_before != protected_after:
        raise ValueError("V9 review changed exact Gate 8 source geometry")
    ...
    blend_path = (
        output_dir
        / "ear-root-wide-spaced-m3-through-bolt-review-v9.blend"
    )
    ...
    report = {
        ...
        "review_sequence": [
            "Review the two maximum-tested safely separated connector sets on each yellow translucent piece.",
    ...
    report_path = (
        output_dir
        / "ear-root-wide-spaced-m3-through-bolt-review-v9-validation.json"
    )
