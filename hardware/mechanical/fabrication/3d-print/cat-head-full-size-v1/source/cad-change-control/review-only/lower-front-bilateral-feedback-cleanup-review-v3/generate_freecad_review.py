#!/usr/bin/env python3
"""Build the consolidated bilateral lower-front cleanup as a fresh V3 review.
...
def load_contract() -> dict[str, Any]:
    ...
    if contract.get("schema_version") != "cat-head-lower-front-bilateral-feedback-cleanup-review-v3":
        raise RuntimeError("unexpected V3 feedback-cleanup contract schema")
    return contract
...
def construct_extra_station_ledge(
    ...
    if copy_offset < 0.0:
        # The requested third station is inside the existing ledge.  Give it
        # only a local positive-depth shelf/root under its 12 mm footprint;
        # do not carry the former outboard extension across the head.
        extension_u_min = station_u - half_u
        extension_u_max = station_u + half_u
    else:
        extension_u_min = (
    ...
    copied_ear = source_ear.copy()
    copied_ear.translate(axis_u * copy_offset)
    # Keep the requested 10 mm clear space between the two 12 mm fastening
    # ears.  Their continuous structural root is the ledge shelf/root below;
    # no intermediate ear copies are allowed in that clear gap.
    integral_extension = copied_ear.copy()
    ...
    # For the inward 22 mm copy, retain the actually clearanced/fused result.
    # Restoring the raw copied ear here would reintroduce the exact rear and
    # neighboring-owner intersections removed above.  The outward prototype
    # remains a separate-solid review path only for historical reproducibility.
    ...
    if len(base_solids) != 1:
        raise RuntimeError(
            f"accepted ledge missing after clearance: {before_bore_solid_volumes}"
        )
    if copy_offset < 0.0:
        candidate = base_solids[0].removeSplitter()
        support_shape = candidate.common(copied_ear).removeSplitter()
    else:
        retained_review_solids = [
            base_solids[0].removeSplitter(),
            copied_ear.removeSplitter(),
        ]
        candidate = Part.makeCompound(retained_review_solids)
        support_shape = copied_ear.copy()
    ...
    base_connected_solid_count = sum(
        1 for solid in connected_solids if v1.common_volume(solid, base) > epsilon
    )
    copied_ear_solid_count = sum(
        1 for solid in connected_solids if v1.common_volume(solid, copied_ear) > epsilon
    )
    if base_connected_solid_count != 1 or copied_ear_solid_count < 1:
        raise RuntimeError(
            f"drilled ledge review lost base or copied ear: {len(connected_solids)} solids"
        )
    candidate = Part.makeCompound([solid.removeSplitter() for solid in connected_solids])
    ...
    metrics = {
        ...
        "review_solid_count": len(connected_solids),
        "base_connected_solid_count": base_connected_solid_count,
        "copied_ear_solid_count": copied_ear_solid_count,
...
def disposable_finalization(shapes: dict[str, Any], contract: dict[str, Any], App: Any) -> dict[str, bool]:
    document = App.newDocument("DISPOSABLE_LOWER_FRONT_FEEDBACK_CLEANUP_V3")
    try:
        selected = [
...
def build() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    manual_owner_raw = manual.copy()
    resolved_manual, manual_audit = v1.resolve_right_lower_ownership(
        [shape_record(
            str(original_map[7]["name"]),
            "RESOLVED LOWER OWNER — RIGHT LOWER — component_007_EXACT_USER_HEAD_BOTTOM_FLANGE_ONLY",
    ...
    evidence = [
        ...
        shape_record("REMOVED_RIGHT_COMPONENT_006", "REMOVED — RIGHT OBSOLETE EYE FLANGE OWNER 006", "cleanup_evidence", original_map[6]["shape"]),
        ...
        shape_record("REMOVED_RIGHT_COMPONENT_023", "REMOVED — RIGHT OBSOLETE UPPER CONNECTOR FLANGE OWNER 023", "cleanup_evidence", original_map[23]["shape"]),
    ...
    unchanged_brep_hashes = {
        ...
        for index in sorted(set(original_map) - {1, 6, 7, 14, 21, 23})
    }
    ...
    manual_extra = component_007.cut(manual)
    manual_extra_volume = 0.0 if manual_extra.isNull() else float(manual_extra.Volume)
    ...
    checks = {
        ...
        "manual_flange_contains_no_legacy_extension": (
            manual_extra_volume <= epsilon
            and abs(float(component_007.Volume) - float(manual_spec["expected_manual_volume_mm3"])) <= 0.001
        ),
        "obsolete_flange_and_reinforcement_owners_are_absent_bilaterally": (
            all(index not in record_map(right_lower) for index in (6, 14, 23))
            and len(right_lower) == len(original_right) - 3
        ),
        ...
        "existing_ledge_and_copied_ear_are_valid_closed_review_solids": (
            right_ledge_shape.isValid() and right_ledge_shape.isClosed()
            and len(right_ledge_shape.Solids) == 1
        ),
        "copied_ear_is_retained_as_separate_review_geometry": (
            station_metrics["base_connected_solid_count"] == 1
            and station_metrics["copied_ear_solid_count"] >= 1
        ...
        "copied_external_ear_through_bore_and_nut_are_accessible": (
            ...
            and abs(station_metrics["ear_copy_offset_mm"] - float(contract["ledge_extension"]["ear_copy_offset_mm"])) <= epsilon
            and abs(
                abs(station_metrics["ear_copy_offset_mm"])
                - float(contract["ledge_extension"]["local_pad_u_width_mm"])
                - float(contract["ledge_extension"]["ear_clear_gap_mm"])
            ) <= epsilon
    ...
    result = {
        "schema_version": "cat-head-lower-front-bilateral-feedback-cleanup-validation-v3",
        "status": status,
        ...
        "measurements": {
            ...
            "manual_flange_extra_mm3": manual_extra_volume,
            "component_006_removed_volume_mm3": float(original_map[6]["shape"].Volume),
            "component_014_removed_volume_mm3": float(original_map[14]["shape"].Volume),
            "component_023_removed_volume_mm3": float(original_map[23]["shape"].Volume),
        ...
        "print_release_holds": [
            "Fresh V3 is review-only and requires user visual approval.",
...
def save_review(result: dict[str, Any], shapes: dict[str, Any]) -> tuple[Path, Path]:
    ...
    if output_dir.exists():
        raise RuntimeError(f"fresh V3 output already exists: {output_dir}")
    output_dir.mkdir(parents=True)
    document = App.newDocument("LOWER_FRONT_BILATERAL_FEEDBACK_CLEANUP_REVIEW_ONLY_V3")
    try:
        names = [
        ...
        decision = document.addObject("App::FeaturePython", "V3_REVIEW_DECISION")
        decision.Label = "REVIEW — CONSOLIDATED LOWER-FRONT CLEANUP AND CONNECTOR LOCATIONS"
