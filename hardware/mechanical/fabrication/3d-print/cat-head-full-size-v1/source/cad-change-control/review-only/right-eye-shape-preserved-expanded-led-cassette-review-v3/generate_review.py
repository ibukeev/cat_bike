...
def similar_scaled_loop(App: Any, loop: Sequence[Any], scale: float) -> list[Any]:
    """Uniformly scale a loop while preserving every edge and corner angle."""
    if not 0.0 < scale <= 1.0:
        raise RuntimeError("visible aperture scale must be in (0, 1]")
    center = App.Vector(
        sum(float(point.x) for point in loop) / len(loop),
        sum(float(point.y) for point in loop) / len(loop),
        sum(float(point.z) for point in loop) / len(loop),
    )
    return [center + (point - center) * float(scale) for point in loop]
...
def construct(context: dict[str, Any], App: Any, Part: Any) -> None:
    ...
    aperture = similar_scaled_loop(
        App, former_aperture, float(values["visible_aperture_uniform_scale"])
    )
    v2_aperture_reference = polygon_inset_loop(
        App,
        former_aperture,
        float(values["superseded_v2_visible_aperture_radial_inset"]),
        origin,
        axis_u,
        axis_v,
        axis_n,
    ...
    lens_surround = toolkit.ring_prism(
        Part,
        outer_front,
        lens_outer,
        float(values["lens_surround_front_depth"]),
        float(values["lens_surround_rear_depth"]),
        axis_n,
        "shape-preserving lens surround",
    )
    walls = toolkit.loft_ring(
        Part, outer_front, inner_front, outer_rear, inner_rear,
        float(values["sidewall_front_depth"]), rear_depth, axis_n,
        "aperture-contained tapered sidewalls",
    )
    seat_outer = polygon_inset_loop(
        App,
        lens_outer,
        -float(values["lens_seat_outer_offset_from_lens"]),
        origin,
        axis_u,
        axis_v,
        axis_n,
    ...
    context.update({
        ...
        "v2_aperture_reference": v2_aperture_reference,
        "aperture": aperture, "lens_outer": lens_outer,
...
def evaluate(context: dict[str, Any]) -> dict[str, Any]:
    ...
    v2_aperture_face = toolkit.polygon_face(Part, context["v2_aperture_reference"])
    aperture_face = toolkit.polygon_face(Part, context["aperture"])
    lens_outer_face = toolkit.polygon_face(Part, context["lens_outer"])
    ...
    uncovered_aperture = float(aperture_face.cut(lens_outer_face).Area)
    expected_scale = float(contract["dimensions_mm"]["visible_aperture_uniform_scale"])
    edge_scale_errors = []
    edge_direction_errors = []
    for index in range(len(context["former_aperture"])):
        former_edge = (
            context["former_aperture"][(index + 1) % len(context["former_aperture"])]
            - context["former_aperture"][index]
        )
        aperture_edge = (
            context["aperture"][(index + 1) % len(context["aperture"])]
            - context["aperture"][index]
        )
        edge_scale_errors.append(
            abs(float(aperture_edge.Length / former_edge.Length) - expected_scale)
        )
        edge_direction_errors.append(
            abs(
                1.0
                - float(aperture_edge.dot(former_edge))
                / float(aperture_edge.Length * former_edge.Length)
            )
        )
    former_center = toolkit.average(context["App"], context["former_aperture"])
    aperture_center = toolkit.average(context["App"], context["aperture"])
    aperture_center_shift = float((aperture_center - former_center).Length)
    aperture_corner_moves = [
        float((source - target).Length)
        for source, target in zip(context["former_aperture"], context["aperture"])
    ]
    aperture_area_gain = (
        float(aperture_face.Area) / float(v2_aperture_face.Area) - 1.0
    )
    ...
    checks = {
        ...
        "visible_aperture_shape_preserved_exactly": (
            max(edge_scale_errors) <= float(gates["maximum_visible_aperture_similarity_error"])
            and max(edge_direction_errors) <= float(gates["maximum_visible_aperture_similarity_error"])
            and aperture_center_shift <= float(gates["maximum_visible_aperture_center_shift_mm"])
        ),
        "visible_aperture_is_larger_than_v2": (
            aperture_area_gain
            >= float(gates["minimum_visible_aperture_area_gain_fraction_over_v2"])
        ),
    ...
    return {
        ...
        "measurements": {
            ...
            "visible_aperture_uniform_scale": expected_scale,
            "visible_aperture_max_edge_scale_error": max(edge_scale_errors),
            "visible_aperture_max_edge_direction_error": max(edge_direction_errors),
            "visible_aperture_center_shift_mm": aperture_center_shift,
            "visible_aperture_corner_moves_toward_shell_mm": aperture_corner_moves,
            "visible_aperture_area_mm2": float(aperture_face.Area),
            "superseded_v2_visible_aperture_area_mm2": float(v2_aperture_face.Area),
            "visible_aperture_area_gain_fraction_over_v2": aperture_area_gain,
...
def create_review(context: dict[str, Any], output: Path) -> Path:
    App = context["App"]
    authority = context["contract"]["authority"]
    doc = App.newDocument("RightEyeShapePreservedExpandedLedCassetteReviewV3")
    try:
        ...
        carrier = add_feature(doc, "PROPOSED__SHAPE_PRESERVED_EXPANDED_EYE_CARRIER", "PROPOSED — LARGER SHAPE-PRESERVED EYE + FITTED PANEL", context["carrier"], (0.12, 0.72, 0.28), 0, authority)
