...
def construct(context: dict[str, Any], App: Any, Part: Any) -> None:
    ...
    carrier = toolkit.fuse_shapes(
        [bezel, walls, lens_seat], "aperture-contained LED cassette carrier"
    ).removeSplitter()
    ...
    context.update({
        ...
        "opening": opening, "former_aperture": former_aperture,
...
def evaluate(context: dict[str, Any]) -> dict[str, Any]:
    ...
    outer_front_face = toolkit.polygon_face(Part, context["outer_front"])
    ...
    outside_opening_area = float(outer_front_face.cut(opening_face).Area)
    opening_gap = float(outer_front_wire.distToShape(opening_wire)[0])
...
def create_review(context: dict[str, Any], output: Path) -> Path:
    App = context["App"]
    authority = context["contract"]["authority"]
    doc = App.newDocument("RightEyeApertureContainedLedCassetteReviewV1")
    try:
        ...
        carrier = add_feature(doc, "PROPOSED__APERTURE_CONTAINED_CARRIER", "PROPOSED — APERTURE-CONTAINED CARRIER", context["carrier"], (0.12, 0.72, 0.28), 0, authority)
        carrier.addProperty("App::PropertyString", "Installation", "ReviewControl")
        carrier.Installation = "Straight through eye opening; black RTV fillet from inside; no exterior flange"
