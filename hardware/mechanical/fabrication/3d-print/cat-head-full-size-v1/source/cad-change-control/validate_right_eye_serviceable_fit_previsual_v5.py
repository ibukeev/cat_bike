#!/usr/bin/env python3
"""Independent fail-closed pre-visual validation for the V5 right eye.
...
ITERATION_ID = "right-eye-serviceable-fit-prototype-v5"
DESIGN_ID = "right-eye-serviceable-fit-B"
TOOLING_REVISION = 1
...
def preflight_independent_root_projection(
    ...
    if rear.get("projection_algorithm") != EDGE_PROJECTION_ALGORITHM:
        raise DesignControlError("V5 edge-projection algorithm mismatch")
    ...
    for role, center in rear["boss_centers_mm"].items():
        ...
        if not passes:
            raise DesignControlError(
                _diagnostic(
                    "independent V5 rear-cap root projection failed",
                    role=role,
...
def preflight_design_control(
    ...
    *,
    ...
    if mutation.get("kind") != "replace_geometry" or mutation.get("object") != TARGET_OBJECT:
        raise DesignControlError("V5 requires one replace_geometry target mutation")
    ...
    if control.get("tooling_revision") != TOOLING_REVISION:
        raise DesignControlError("tooling_revision must remain 1 for this tooling draft")
    ...
    for item in defect_map:
        if not item.get("v5_construction") or not item.get("validation_gate"):
            raise DesignControlError(
...
def _mount_frames(parameters: dict[str, Any], App: Any) -> dict[str, dict[str, Any]]:
    ...
    outer = _planar_loop(
        [_vector(App, point) for point in parameters["geometry"]["shell_opening_boundary_mm"]],
        origin,
        axis_u,
        axis_v,
        axis_n,
    ...
    for role, datum in parameters["mount_datums"].items():
        ...
        tangent = _normalized(App, tangent, f"{role} tangent")
        radial = _normalized(App, axis_n.cross(tangent), f"{role} radial")
        if radial.dot(aperture_midpoint - anchor) < 0.0:
            radial = -radial
        expected_axis = _normalized(App, _vector(App, datum["bore_axis"]), f"{role} datum axis")
        if radial.dot(expected_axis) < 0.999999:
            raise RuntimeError(f"{role} reconstructed V5 axis differs from approved datum")
...
def _reference_geometry(parameters: dict[str, Any], App: Any, Part: Any) -> dict[str, Any]:
    ...
    for role, frame in frames.items():
        eye_center = frame["eye_bore"] - frame["axis_n"] * (
        ...
        leaf = _oriented_box(
            Part,
            thick_center,
            (frame["tangent"], frame["axis_n"], frame["radial"]),
        ...
        bore = Part.makeCylinder(
            ...
            frame["eye_bore"]
            - frame["radial"] * (float(head_mount["bore_cutter_length_mm"]) / 2.0),
            frame["radial"],
        )
        drilled = extended.cut(bore).removeSplitter()
        ...
        annulus = annulus_outer.cut(
            Part.makeCylinder(bore_radius, thickness, collar_base, frame["radial"])
        )
        mounts[role] = {
            "uncut": extended,
            "drilled": drilled,
...
def _hardware_envelopes(parameters: dict[str, Any], references: dict[str, Any], App: Any, Part: Any) -> dict[str, dict[str, Any]]:
    ...
    for role, mount in references["mounts"].items():
        ...
        eye_owner_face = eye_bore + radial * (thickness - mating_offset)
        head_owner_face = head_bore - radial * (thickness - mating_offset)
        ...
        head_washer = Part.makeCylinder(
            washer_radius,
            washer_thickness,
            head_owner_face - radial * washer_thickness,
            radial,
        )
        ...
        nyloc = Part.makeCylinder(
            ...
            head_owner_face
            - radial * (washer_thickness + float(hardware["nyloc_length_mm"])),
            radial,
        )
...
def measure_previsual(
    ...
    for role, mount in references["mounts"].items():
        ...
        measured_mating_gap = (
            (mount["eye_bore"] - mount["head_bore"]).dot(mount["radial"])
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = {
        ...
        "design_id": DESIGN_ID,
        "tooling_revision": TOOLING_REVISION,
        "validator": "independent-right-eye-serviceable-fit-previsual-v5",
