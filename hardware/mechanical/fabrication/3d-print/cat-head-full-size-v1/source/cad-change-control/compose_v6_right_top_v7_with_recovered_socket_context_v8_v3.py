"""Compose V7 with the exact V5 V34 right rail socket/root context for review.

This intentionally does *not* global-fuse the entire multipart top shell: that
Boolean made an invalid non-manifold result.  C002 and its known C042 root are
fused locally; the remaining independently valid V34 context solids are kept
in their exact V5 locations in a review-only compound.  C020 is deliberately
...
OUTPUT_NAME = "PROPOSED_TOP_RIGHT_PRINT_PACKAGE_V8_SOCKET_CONTEXT_COMPOUND"
EXPECTED = (
...
for source_name in EXPECTED:
    ...
    if output is None:
        ...
        output.Status = "FROZEN_EXACT_V5_REFERENCE_ONLY__NOT_PRINT_RELEASED"
...
socket_root = c002.Shape.fuse(c042.Shape)
valid(socket_root, "C002/C042 local fuse is invalid.")

context_shapes = [obj.Shape for obj in recovered if obj not in (c002, c042)]
package_shape = Part.makeCompound([source.Shape, socket_root] + context_shapes)
valid(package_shape, "V8 socket-context compound is invalid.")
...
output.Shape = package_shape
output.Label = "PROPOSED TOP-RIGHT — V8 SOCKET CONTEXT COMPOUND — REVIEW ONLY"
output.SourcePackage = source
output.RecoveredSocketContext = recovered
output.Status = "REVIEW_ONLY__V7_PLUS_EXACT_V5_SOCKET_CONTEXT__NO_GLOBAL_BOOLEAN__NOT_PRINT_RELEASED"
output.Contract = "V7 + exact V5 C002/C022/C023/C033/C034/C037/C038/C042; C020 and C032 excluded"
...
print("V7 solids: %d | V8 package solids: %d" % (len(source.Shape.Solids), len(output.Shape.Solids)))
print("C002/C042 common volume: %.6f mm3" % c002_c042)
print("V7 and all frozen sources unchanged; V6 not saved or print-released.")
