#!/usr/bin/env python3
"""Generate one V5 serviceable-fit right-eye candidate from canonical V34.

This is a new construction lineage.  It does not import, call, inspect, or
reuse the V1/V3 generator, any V1/V3 candidate, or the quarantined V4 FCStd.
The eye is reconstructed
from the protected aperture/LCS, the numeric V5/V17 mount datums, and the V5
...
import validate_right_eye_serviceable_fit_previsual_v5 as design_control
    ...
    for role, center_values in rear["boss_centers_mm"].items():
        ...
        if failures:
            raise RuntimeError(
                json.dumps(
                    {
                        "error": "V5 rear-cap edge-projection preflight failed",
                        "role": role,
                        "measurements": record,
                        "output_created": False,
                        "geometry_construction_started": False,
...
def immutable_preflight(
    ...
    if len(mutations) != 1 or not isinstance(mutations[0].get("parameters"), dict):
        raise RuntimeError("V5 requires one machine-readable mutation")
    ...
    shared_validator = import_pinned_module(
        shared_validator_path, "cat_head_shared_v2_validator_for_eye_v5"
    )
    ...
    probe = import_pinned_module(probe_path, "cat_head_runtime_probe_for_eye_v5")
...
def construct_serviceable_eye(parameters: dict[str, Any], App: Any, Part: Any) -> tuple[Any, dict[str, Any]]:
    ...
    require_single_solid(final, "final V5 serviceable eye")
    construction = {
        "origin": origin,
        "axis_u": axis_u,
        "axis_v": axis_v,
...
def attach_history(target: Any, parameters: dict[str, Any], construction: dict[str, Any]) -> None:
    for name, value in (
        ...
        add_property(target, "App::PropertyString", name, "ServiceableFitB")
    ...
    for role, record in construction["cap_roots"].items():
        ...
        for suffix, value in (
            ...
            add_property(
                target,
                "App::PropertyLength" if suffix != "ProjectionSegmentParameter" else "App::PropertyFloat",
                name,
                "ServiceableFitDimensions",
            )
            setattr(target, name, float(value))
...
def construct_candidate(context: dict[str, Any]) -> None:
    ...
    print("STAGE 1/5 immutable V5 preflight passed and saveAs completed", flush=True)
    ...
    require_single_solid(target.Shape, "saved V5 target")
    document.save()
