#!/usr/bin/env python3
"""Independent fail-closed pre-visual validation for the V4 right eye.
...
def preflight_design_control(
    ...
    *,
    ...
    if mutation.get("kind") != "replace_geometry" or mutation.get("object") != TARGET_OBJECT:
        raise DesignControlError("V4 requires one replace_geometry target mutation")
    ...
    for item in defect_map:
        if not item.get("v4_construction") or not item.get("validation_gate"):
            raise DesignControlError(
...
def _reference_geometry(parameters: dict[str, Any], App: Any, Part: Any) -> dict[str, Any]:
    ...
    for role, center_values in rear["boss_centers_mm"].items():
        ...
        if span > float(rear["maximum_local_root_span_mm"]):
            raise RuntimeError(f"{role} rear-cap connector root span exceeds local limit")
        if span > 1.0e-9:
            ...
            cap_shapes.append(
                _face(Part, loop).extrude(axis_n * float(rear["engagement_depth_mm"]))
            )
...
def _metadata_exact(target: Any, parameters: dict[str, Any], App: Any) -> dict[str, bool]:
    ...
    return {
        ...
        "connector_dimensions_exact": dimensions_ok,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = {
        ...
        "design_id": DESIGN_ID,
        "tooling_revision": TOOLING_REVISION,
        "validator": "independent-right-eye-serviceable-fit-previsual-v4",
