#!/usr/bin/env python3
"""Real Prusa MK4 Generic ASA slice audit for all thirteen V9 printed parts."""
...
DEFAULT_V9_DIR = (
    PACKAGE_ROOT / "output/gate9-body-seam-retention-candidate-v9"
)
DEFAULT_CONFIG = (
    PACKAGE_ROOT
...
DEFAULT_REVIEW = (
    PACKAGE_ROOT / "review/gate9-body-seam-retention-v9-summary.json"
)
PART_ORDER = (
    "left_upper_head",
    "right_upper_head",
    ...
    "left_socket_cap",
    "right_socket_cap",
    "body_seam_bridge__left_lower_face__left_upper_head_02",
    "body_seam_bridge__left_lower_face__right_lower_face_05",
    "body_seam_bridge__left_upper_head__right_upper_head_02",
    "body_seam_bridge__left_upper_head__right_upper_head_03",
    "body_seam_bridge__right_lower_face__right_upper_head_02",
)
...
def slice_part(
    ...
    candidates = []
    print(
        f"[slice] gate9_v9_body_retention/{part}: "
    ...
    for index, orientation in enumerate(orientations, start=1):
        ...
        candidates.append(
            comparison.slice_orientation(
                stl_path,
                orientation,
                output_dir / "gate9_v9_body_retention" / part,
                config_path,
                threads,
                index,
            )
...
def main() -> None:
    args = parse_args()
    v9_dir = args.v9_dir.resolve()
    config_path = args.config.resolve()
    review_path = args.review.resolve()
    output_dir = v9_dir / "slicer-review"
    output_dir.mkdir(parents=True, exist_ok=True)
    paths = {
        part: v9_dir / "parts" / f"{part}.stl"
    ...
    totals["summation_method"] = (
        ...
        "four V9 body shells, rear bezel, bottom keel, two socket caps, "
        "and five removable body-seam bridges"
    )
    validation = {
        "all_thirteen_v9_parts_have_margin_passing_slice": (
    ...
    report = {
        "status": (
            "digital V9 support/brim-inclusive Generic ASA feasibility on "
        ...
        "parts": part_reports,
        "exact_thirteen_part_set": totals,
        "validation": validation,
    }
    report_path = output_dir / "gate9-v9-body-retention-slices.json"
    ...
    review["digital_validation"][
        "digital_v9_body_retention_and_slice_candidate_pass"
