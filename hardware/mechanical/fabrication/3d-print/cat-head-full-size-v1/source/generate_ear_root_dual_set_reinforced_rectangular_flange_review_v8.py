#!/usr/bin/env python3
"""Generate two reinforced connector sets per mirrored translucent ear piece."""
...
DEFAULT_CONFIG = (
    PACKAGE_ROOT / "config/ear-root-dual-set-reinforced-rectangular-flange-review-v8.json"
)
...
def configure_scene(output_dir: Path, resolution_px: int) -> bpy.types.Object:
    ...
    scene.name = "Ear_Root_Internal_Rectangular_Flange_Placement_Review_V8"
    ...
    camera_data = bpy.data.cameras.new("EAR8_REVIEW_ONLY__Camera")
    camera = bpy.data.objects.new("EAR8_REVIEW_ONLY__Camera", camera_data)
...
def render_view(
    ...
    path = (
        output_dir
        / "renders"
        / f"ear-root-dual-set-reinforced-rectangular-{name}.png"
    )
...
def create_pair(
    ...
    orange = gate5.box(
        f"EAR8_{label}_INSERT_FLANGE__orange",
        orange_center,
        axes,
        dimensions,
        materials["orange"],
    ...
    green = gate5.box(
        f"EAR8_{label}_HEAD_FLANGE__green",
        green_center,
        axes,
        dimensions,
        materials["green"],
    )
    gate5.require_manifold(orange, f"V8 {label} orange rectangular tab")
    gate5.require_manifold(green, f"V8 {label} green rectangular tab")
    ...
    if c002_v2.surfaces_overlap(orange, green):
        raise ValueError(f"V8 {label} rectangular tabs overlap")
    measured_gap = c002_v2.surface_distance(orange, green)
    if abs(measured_gap - gap) > 0.01:
        raise ValueError(
            f"V8 {label} placement gap changed: expected {gap}, got {measured_gap}"
        )
    ...
    if orange_owner_hits == 0:
        raise ValueError(f"V8 {label} orange tab does not root into moving owner")
    if green_owner_hits == 0:
        raise ValueError(f"V8 {label} green tab does not root into fixed owner")
...
def mirror_pair_from_right(
    ...
    for role, material_name in (("orange", "orange"), ("green", "green")):
        ...
        mirrored = ear_v2.deserialize_mesh(
            f"EAR8_LEFT_{location_id}_{role.upper()}_FLANGE__{role}",
    ...
    if orange_hits == 0 or green_hits == 0:
        raise ValueError(
            f"V8 LEFT_{location_id} mirrored owner root missing: "
...
def create_boolean_proof_part(
    ...
    if not result.data.polygons or gate5.mesh_volume(result) <= 0.001:
        raise ValueError(f"V8 review proof is empty: {name}")
...
def fuse_owner_with_tabs(
    ...
    if (
        ...
        raise ValueError(f"V8 owner/tab composite is not manifold: {topology}")
...
def validate_side_path(
    ...
    deep_envelope = gate8.expanded_insert_cutter(
        deep_body, deep_margin, f"EAR8_VALIDATION__{side}_deep_body_margin"
    )
    flange_envelopes = []
    for orange in orange_tabs:
        envelope = gate8.expanded_insert_cutter(
            orange,
            flange_margin,
            f"EAR8_VALIDATION__{orange.name}_margin",
        )
    ...
    if actual_conflicts:
        raise ValueError(f"V8 {side} actual path collides: {actual_conflicts}")
    return {
        "side": side,
...
def main() -> None:
    ...
    right_deep = ear_v2.deserialize_mesh(
        "EAR8_VALIDATION__right_deep_fit_body", source_payloads["deep"]
    )
    bpy.context.scene.collection.objects.link(right_deep)
    right_body = ear_v2.deserialize_mesh(
        "EAR8_ACCEPTED_V3_BODY__right", source_payloads["full"]
    )
    ...
    left_deep = ear_v2.deserialize_mesh(
        "EAR8_VALIDATION__left_deep_fit_body",
    ...
    if len(location_specs) != 2:
        raise ValueError("V8 requires exactly two connector locations per side")
    ...
    for spec in location_specs:
        ...
        if right_record["owner"] != "right_upper_head":
            raise ValueError(
                f"V8 right {spec['id']} owner changed: {right_record['owner']}"
            )
        ...
        if left_record["owner"] != "left_upper_head":
            raise ValueError(
                f"V8 left {spec['id']} owner changed: {left_record['owner']}"
            )
        ...
        if mirrored_anchor_error > 0.01:
            raise ValueError(
                f"V8 left {spec['id']} seam anchor is not mirrored: "
    ...
    if len(pairs) != 4:
        raise ValueError("V8 must contain four connector sets total")
    separations = {}
    ...
    for side, side_pairs in pairs_by_side.items():
        ...
        if separation < minimum_separation:
            raise ValueError(
                f"V8 {side} connector sets are clustered: {separation:.3f} mm"
            )
    mirror_errors = {}
    for spec in location_specs:
        right_pair = next(
        ...
        if error > 0.01:
            raise ValueError(
                f"V8 location {spec['id']} is not mirrored: {error:.4f} mm"
            )
    ...
    for pair in pairs:
        ...
        prefix = f"EAR8_REVIEW_ONLY__{key}"
        parts = {
        ...
        for owner_key in ("orange_owner_overlap", "green_owner_overlap"):
            ...
            if volume < minimum_overlap:
                raise ValueError(
                    f"V8 weak owner root {volume_key}: {volume:.3f} mm3"
                )
    ...
    for side in ("right", "left"):
        ...
        composite, topology = fuse_owner_with_tabs(
            bodies[side],
            orange_tabs,
            f"EAR8_VALIDATION__{side}_body_plus_two_orange_tabs",
        )
        ...
        if side_seated_hits:
            raise ValueError(
                f"V8 {side} moving composite hits shells: {side_seated_hits}; "
        ...
        if per_green_hits:
            raise ValueError(
                f"V8 {side} green tabs hit unintended shells: {per_green_hits}"
            )
    ...
    collection_names = (
        ...
        "EAR8_OTHER_SOURCE_GEOMETRY__HIDDEN",
    )
    ...
    for target in structural_targets:
        ...
        c002_v2.link_reference(
            target, collections["EAR8_EXACT_STRUCTURAL_HEAD_MUTED__UNCHANGED"]
        )
    ...
    for ear in ears.values():
        ...
        ear.hide_set(False)
        c002_v2.link_reference(
            ear, collections["EAR8_EXACT_EARS_CYAN__UNCHANGED"]
        )
    for body in bodies.values():
        c002_v2.link_reference(
            body,
    ...
    for pair in pairs:
        ...
        c002_v2.link_reference(
            pair["orange"],
            collections[f"EAR8_{side_label}_INSERT_FLANGES_ORANGE__TWO_SETS"],
        )
        c002_v2.link_reference(
            pair["green"],
            collections[
                f"EAR8_{side_label}_HEAD_FLANGES_GREEN__TWO_SETS_UNINTEGRATED"
            ],
        )
    ...
    for obj in source_set:
        if obj in default_visible:
            continue
        c002_v2.link_reference(
            obj, collections["EAR8_OTHER_SOURCE_GEOMETRY__HIDDEN"]
        )
    ...
    for view_name, (baseline_render, candidate_render) in (
        ...
        if not comparison["identical"]:
            raise ValueError(
                f"V8 changes the {view_name} exterior silhouette: {comparison}"
            )
    ...
    if protected_before != protected_after:
        raise ValueError("V8 review changed exact Gate 8 source geometry")
    ...
    blend_path = (
        output_dir
        / "ear-root-dual-set-reinforced-rectangular-flange-review-v8.blend"
    )
    ...
    report = {
        ...
        "review_sequence": [
            "Review two separated connector sets on each yellow translucent piece.",
            "Review all eight plain rectangular tabs and the owner-root cutaways.",
            "Do not add holes or hardware until placement and conservative clearance are accepted.",
        ],
    ...
    report_path = (
        output_dir
        / "ear-root-dual-set-reinforced-rectangular-flange-review-v8-validation.json"
    )
