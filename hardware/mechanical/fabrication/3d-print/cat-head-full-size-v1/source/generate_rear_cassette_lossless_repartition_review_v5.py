...
V5 does not Boolean-cut the Gate 8 lower-face objects. It regenerates four
closed review shells from complementary source-facet sets and restores the
accepted Gate 8 lower-owned opaque muzzle panels: retained left and
...
def clean_gate8_scene() -> tuple[list[str], dict[str, str]]:
    ...
    for name in (*LOWER_SECTIONS, "rear_base"):
        ...
        obj["candidate_view_status"] = (
            "unchanged hidden source; V5 complementary review shells shown"
        )
...
def create_closed_shell(
    ...
    solidify = obj.modifiers.new("V5_INWARD_SHELL_1P8MM", "SOLIDIFY")
...
def create_rear_base_review_copy(
    ...
    review.name = "V5_CASSETTE__unchanged_rear_base"
    ...
    review["meaning"] = "unchanged Gate 8 rear_base in V5 cassette ownership"
    return review
...
def face_ledger(
    model: gate1.ObjModel,
    source_by_section: dict[str, set[int]],
    points: list[tuple[float, float, float]],
    ...
    for section in LOWER_SECTIONS:
        all_indices = source_by_section[section]
...
def configure_scene(output_dir: Path, resolution_px: int) -> bpy.types.Object:
    ...
    scene.name = "Rear_Cassette_Lossless_Repartition_Review_V5"
...
def main() -> None:
    ...
    gate8_config = json.loads(
        repo_path(config["source_gate8_config"]).read_text(encoding="utf-8")
    )
    restored_panel_ids_by_section = {
        section: set(
            config["restored_gate8_lower_opaque_panels"][section]
        )
        for section in LOWER_SECTIONS
    }
    accepted_gate8_panel_ids_by_section = {
        section: set(
            gate8_config["opaque_muzzle_frame"]["opaque_panel_groups"][
                section
            ]
        )
        for section in LOWER_SECTIONS
    ...
    if (
        ...
        raise ValueError(
            "V5 lower opaque-panel ownership does not match Gate 8: "
            f"{restored_panel_ids_by_section} != "
            f"{accepted_gate8_panel_ids_by_section}"
        )
    panel_owner = {
        panel_id: section
        for section, panel_ids in restored_panel_ids_by_section.items()
        for panel_id in panel_ids
    }
    opaque_indices_by_section = {section: set() for section in LOWER_SECTIONS}
    for index, face in enumerate(model.faces):
        ...
        if assignments[index] != "removable_glow":
            raise ValueError(
                f"Gate 8 opaque panel {panel_id} unexpectedly has "
                f"assignment {assignments[index]}"
            )
        opaque_indices_by_section[owner].add(index)
    if any(
        len(indices) != 3
        for indices in opaque_indices_by_section.values()
    ):
        raise ValueError(
            "V5 expected three reclassified source faces per lower side "
            f"(one split quad plus one triangle): {opaque_indices_by_section}"
        )
    ...
    all_by_section = {
        section: (
            structural_by_section[section]
            | opaque_indices_by_section[section]
        )
        for section in LOWER_SECTIONS
    }
    selected_by_section = {
    ...
    if any(len(indices) != 5 for indices in selected_by_section.values()):
        raise ValueError(
            "V5 requires exactly the approved five moved facets per side: "
    ...
    if (
        ...
        raise ValueError(f"V5 exterior face ledger is not lossless: {ledger}")

    retained_collection = bpy.data.collections.new("V5_RETAINED_LOWER_EXTERIORS")
    cassette_collection = bpy.data.collections.new(
        "V5_ENLARGED_REAR_CASSETTE_OWNERSHIP"
    )
    seam_collection = bpy.data.collections.new("V5_REPARTITION_BOUNDARIES")
    ...
    for section in LOWER_SECTIONS:
        retained, retained_metrics = create_closed_shell(
            f"V5_RETAINED__{section}_exterior_shell",
            model,
            points,
        ...
        moved, moved_metrics = create_closed_shell(
            f"V5_CASSETTE__moved_from_{section}",
            model,
            points,
    ...
    blend_path = output_dir / "rear-cassette-lossless-repartition-review-v5.blend"
    ...
    for obj in export_objects:
        ...
        obj.select_set(True)
    glb_path = output_dir / "rear-cassette-lossless-repartition-review-v5.glb"
    ...
    report = {
        ...
        "repartition_equation": (
            "original lower exteriors including accepted Gate 8 opaque muzzle "
            "panels = retained left/right lower shells + moved left/right "
            "cassette shells"
        ),
        ...
        "restored_gate8_lower_opaque_panel_ids_by_section": {
            section: sorted(panel_ids)
            for section, panel_ids in restored_panel_ids_by_section.items()
        },
        "restored_gate8_lower_opaque_face_count_by_section": {
            section: len(indices)
            for section, indices in opaque_indices_by_section.items()
        },
        ...
        "reinforcement_status": (
            ...
            "hidden source objects; V5 makes no reinforcement ownership claim"
        ),
    ...
    report_path = output_dir / (
        "rear-cassette-lossless-repartition-review-v5-validation.json"
    )
