#!/usr/bin/env python3
"""Generate the finalized aluminum-side CAT-HEAD-SHELL-ALUMINUM-V0.5 handoff.
...
Run against the locked V6.1 socket assembly:
    ...
    blender --background \
      ...
      --python source/generate_frame_fixed_mount_v05.py
...
CONFIG_PATH = PACKAGE_ROOT / "config/frame-fixed-mount-v05-final.json"
REVIEW_PATH = PACKAGE_ROOT / "review/frame-fixed-mount-v05-final-summary.json"
...
def load_inputs() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if interface["interface_revision"] != config["required_interface_revision"]:
        raise ValueError("V0.5 generator received the wrong shared interface")
    if socket_summary["interface_revision"] != config["required_socket_interface_revision"]:
...
def write_backplate_outputs(
    ...
    body += svg_text(
        -59,
        49,
        "V0.5 HEAD BACKPLATE | 3 mm 6061-T6 | PRINT 100%",
    )
    body += svg_text(
        -59,
        45,
    ...
    body += svg_text(
        -59,
        -48,
        "V0.5-M2 ORDERED-ANGLE METAL-SIDE PATTERN; PHYSICAL COUPON + ASA INTEGRATION REQUIRED",
        "warn",
    )
    (paths["flat"] / "head-rear-backplate-v05-1to1.svg").write_text(
        svg_document(
            320.0,
            115.0,
            body,
            "V0.5-M2 ordered-angle aluminum backplate 1:1",
        ),
        encoding="utf-8",
    )
    ...
    (paths["flat"] / "head-rear-backplate-v05.dxf").write_text(
        "\n".join(dxf) + "\n",
        encoding="ascii",
    )
...
def write_rail_output(
    ...
    (paths["rails"] / "rail-cut-and-drill-v05-m2-1to1.svg").write_text(
        svg_document(330.0, 82.0, body, "V0.5-M2 rail compound cut and drill drawing"),
        encoding="utf-8",
    )
    wrap_body = ""
    ...
    (paths["rails"] / "rail-lower-compound-wrap-v05-m2-1to1.svg").write_text(
        svg_document(330.0, 155.0, wrap_body, "V0.5-M2 compound lower-cut wrap template"),
        encoding="utf-8",
    )
...
def write_angle_connector_output(
    ...
    (paths["formed"] / "lower-angle-connector-v05-m2-plan.svg").write_text(
        svg_document(340.0, 90.0, body, "V0.5-M2 ordered-angle connector plan"),
        encoding="utf-8",
    )
...
def base_validation(
    ...
    expected_bottom_centers = [[-7.4, -30.0], [7.4, -30.0]]
    bottom_centers = sorted(
        [list(point) for point in shell_holes(config) if float(point[1]) == -30.0]
    )
    washer_diameter = float(
        config["backplate"]["shell_attachment"]["washer_outer_diameter_mm"]
    )
    tool_diameter = float(
        config["backplate"]["shell_attachment"][
            "shell_pad_tool_envelope_diameter_mm"
        ]
    )
    bottom_washer_edge_margin = min(
        edge_ligament(point, washer_diameter, outline) for point in bottom_centers
    )
    bottom_tool_edge_margin = min(
        edge_ligament(point, tool_diameter, outline) for point in bottom_centers
    )
    opposing_bottom_tool_gap = (
        math.dist(bottom_centers[0], bottom_centers[1]) - tool_diameter
    )
    bottom_tool_to_adapter_hole_clearance = min(
        math.dist(bottom, adapter)
        - tool_diameter / 2.0
        - adapter_diameter / 2.0
        for bottom in bottom_centers
        for adapter in adapter_holes(interface)
    )
    ...
    checks = {
        "interface_revision_is_v05": (
            interface["interface_revision"] == "CAT-HEAD-SHELL-ALUMINUM-V0.5"
        ),
        ...
        "only_bottom_shell_centers_use_coordinated_v05_positions": (
            [list(point) for point in shell_holes(config)]
            == [
                [-10.0, 30.0],
                [10.0, 30.0],
                [-20.0, 0.0],
            ...
            and bottom_centers == expected_bottom_centers
        ),
        "bottom_shell_washers_clear_backplate_edge": (
            bottom_washer_edge_margin
            >= float(config["validation"]["minimum_bottom_shell_washer_edge_margin_mm"])
        ),
        "bottom_shell_tools_clear_backplate_edge": (
            bottom_tool_edge_margin
            >= float(config["validation"]["minimum_bottom_shell_tool_edge_margin_mm"])
        ),
        "opposing_bottom_shell_tools_clear_each_other": (
            opposing_bottom_tool_gap
            >= float(config["validation"]["minimum_opposing_bottom_shell_tool_gap_mm"])
        ),
        "bottom_shell_tools_clear_adapter_holes": (
            bottom_tool_to_adapter_hole_clearance
            >= float(config["validation"]["minimum_bottom_tool_to_adapter_hole_clearance_mm"])
        ),
    ...
    return {
        ...
        "derived": {
            ...
            "coordinated_bottom_shell_centers_mm": bottom_centers,
            "minimum_bottom_shell_washer_edge_margin_mm": round(
                bottom_washer_edge_margin, 4
            ),
            "minimum_bottom_shell_tool_edge_margin_mm": round(
                bottom_tool_edge_margin, 4
            ),
            "opposing_bottom_shell_tool_gap_mm": round(
                opposing_bottom_tool_gap, 4
            ),
            "minimum_bottom_tool_to_adapter_hole_clearance_mm": round(
                bottom_tool_to_adapter_hole_clearance, 4
            ),
...
def blender_geometry(
    ...
    try:
        ...
        raise RuntimeError(
            "V0.5-M2 generation requires Blender and the locked V6.1 shell"
        ) from error
    ...
    plate_material = material(
        "metal_v05_m2_backplate", (0.48, 0.58, 0.68, 1.0), 0.8
    )
    rail_material = material(
        "metal_v05_m2_rails", (0.12, 0.37, 0.74, 1.0), 0.75
    )
    angle_material = material(
        "metal_v05_m2_ordered_angle", (0.95, 0.48, 0.05, 1.0), 0.75
    )
    plug_material = material(
        "metal_v05_m2_solid_plug", (0.96, 0.78, 0.12, 1.0), 0.72
    )
    spacer_material = material(
        "metal_v05_m2_taper_spacers", (0.82, 0.68, 0.16, 1.0), 0.68
    )
    hardware_material = material(
        "metal_v05_m2_hardware", (0.10, 0.10, 0.12, 1.0), 0.9
    )
    ...
    for group, points, diameter in hole_sets:
        for index, (x_value, v_value) in enumerate(points):
            cut_cylinder(
                backplate,
                f"metal_v05__backplate_{group}_{index:02d}",
    ...
    for side in ("left", "right"):
        ...
        for index, offset in enumerate(crossbolt_offsets):
            ...
            cut_cylinder(
                rail,
                f"metal_v05__rail_{side}_lower_m5_{index:02d}",
        ...
        cut_cylinder(
            rail,
            f"metal_v05__rail_{side}_upper_m4",
        ...
        for index, offset in enumerate(crossbolt_offsets):
            ...
            cut_cylinder(
                plug,
                f"metal_v05__plug_{side}_m5_{index:02d}",
        ...
        angle_upright = oriented_box(
            f"metal_v05__angle_upright_{side}",
            upright_center,
            (q_axis, s_axis, normal),
        ...
        for index, (x_value, v_value) in enumerate(current_holes):
            ...
            cut_cylinder(
                angle_base,
                f"metal_v05__angle_base_{side}_m5_{index:02d}",
                hole_point + normal * 7.0,
                hole_point - normal * 7.0,
                float(config["backplate"]["shoe_attachment"]["plate_clearance_diameter_mm"]),
            )
            plate_fastener = gate5.cylinder(
                f"metal_v05__plate_m5_{side}_{index:02d}",
        ...
        for index, offset in enumerate(crossbolt_offsets):
            ...
            cut_cylinder(
                angle_upright,
                f"metal_v05__angle_upright_{side}_m5_{index:02d}",
        ...
        outer_cheek = oriented_box(
            f"metal_v05__outer_cheek_{side}",
            cheek_center,
            (outward, other, axis),
            (
        ...
        for index, offset in enumerate(crossbolt_offsets):
            ...
            cut_cylinder(
                outer_cheek,
                f"metal_v05__outer_cheek_{side}_m5_{index:02d}",
        ...
        for index, (offset, thickness_range) in enumerate(
            ...
            spacer = oriented_box(
                f"metal_v05__taper_spacer_{side}_{index:02d}",
                hole_center
            ...
            bolt = gate5.cylinder(
                f"metal_v05__crossbolt_{side}_{index:02d}",
            ...
            head = gate5.cylinder(
                f"metal_v05__crossbolt_head_{side}_{index:02d}",
            ...
            nut = gate5.cylinder(
                f"metal_v05__crossbolt_nut_{side}_{index:02d}",
    ...
    render(
        "v05-m2-shell-integration-rear",
        (245.0, 520.0, 225.0),
        (0.0, 247.0, 168.0),
    )
    render(
        "v05-m2-angle-frame-rear",
        (210.0, 500.0, 210.0),
        (0.0, 250.0, 165.0),
        hidden=shell_objects,
    )
    render(
        "v05-m2-angle-frame-front",
        (205.0, 65.0, 190.0),
        (0.0, 248.0, 164.0),
        hidden=shell_objects,
    )
    render(
        "v05-m2-right-connector-detail",
        (125.0, 170.0, 126.0),
        (40.0, 258.0, 150.0),
        hidden=shell_objects
        | {
    ...
    render(
        "v05-m2-right-connector-internal",
        (112.0, 188.0, 120.0),
        (40.0, 258.0, 151.0),
        hidden=shell_objects
        | {
    ...
    model_path = paths["model"] / "frame-fixed-mount-v05-m2-angle-stock-review.blend"
...
def main() -> None:
    ...
    report = {
        "schema_version": 1,
        "status": (
            "PASS - V0.5-M2 ORDERED-ANGLE ALUMINUM HANDOFF GENERATED"
            if passed
            else "FAIL"
        ),
        "release_status": (
        ...
        "review_outputs": {
            "backplate_svg": str(
                (
                    paths["flat"]
                    / "head-rear-backplate-v05-1to1.svg"
                ).relative_to(REPO_ROOT)
            ),
            "backplate_dxf": str(
                (
                    paths["flat"] / "head-rear-backplate-v05.dxf"
                ).relative_to(REPO_ROOT)
            ),
            "rail_drawing": str(
                (
                    paths["rails"]
                    / "rail-cut-and-drill-v05-m2-1to1.svg"
                ).relative_to(REPO_ROOT)
            ),
            "rail_wrap_template": str(
                (
                    paths["rails"]
                    / "rail-lower-compound-wrap-v05-m2-1to1.svg"
                ).relative_to(REPO_ROOT)
            ),
            "angle_connector_drawing": str(
                (
                    paths["formed"]
                    / "lower-angle-connector-v05-m2-plan.svg"
                ).relative_to(REPO_ROOT)
            ),
            "review_blend": str(
                (
                    paths["model"]
                    / "frame-fixed-mount-v05-m2-angle-stock-review.blend"
                ).relative_to(REPO_ROOT)
            ),
            "renders": [
                str(path.relative_to(REPO_ROOT))
                for path in sorted(paths["renders"].glob("v05-m2-*.png"))
            ],
        },
        "release_holds": config["release_holds"],
    }
    serialized = json.dumps(report, indent=2) + "\n"
    (paths["validation"] / "frame-fixed-mount-v05-validation.json").write_text(
        serialized,
        encoding="utf-8",
    )
    ...
    tracked_collision["full_collision_record_location"] = str(
        (
            paths["validation"]
            / "frame-fixed-mount-v05-validation.json"
        ).relative_to(REPO_ROOT)
    )
