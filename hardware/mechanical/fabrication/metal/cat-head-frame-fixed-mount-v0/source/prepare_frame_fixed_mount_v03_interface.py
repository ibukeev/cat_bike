#!/usr/bin/env python3
"""Resolve aluminum mount V0.3 review inputs from the shared interface.
...
COORDINATOR_PATH = PACKAGE_ROOT / "config/frame-fixed-mount-v03-interface.json"
...
def load_resolved_config() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if not all(checks.values()):
        ...
        raise ValueError(f"Metal V0.3 interface preflight failed: {failed}")
    report = {
        "status": "PASS - METAL V0.3 SHARED INTERFACE PREFLIGHT",
