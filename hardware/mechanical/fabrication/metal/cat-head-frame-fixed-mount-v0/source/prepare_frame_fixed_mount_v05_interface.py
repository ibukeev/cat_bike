#!/usr/bin/env python3
"""Resolve aluminum mount V0.5 review inputs from the shared interface.

This preflight resolves the V0.5-M2 ordered-angle handoff and performs no
...
COORDINATOR_PATH = PACKAGE_ROOT / "config/frame-fixed-mount-v05-interface.json"
...
def load_resolved_config() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    checks = {
        ...
        "final_config_uses_v05_interface": (
        ...
        "final_shell_pattern_matches_coordinated_v05_centers": (
            final_config["backplate"]["shell_attachment"][
                "local_x_v_centers_mm"
            ]
            == interface["aluminum_backplate"][
    ...
    if not all(checks.values()):
        ...
        raise ValueError(f"Metal V0.5 interface preflight failed: {failed}")
    report = {
        "status": "PASS - METAL V0.5-M2 ANGLE-STOCK SHARED INTERFACE PREFLIGHT",
